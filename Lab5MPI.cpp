...
{	//теги сообщений 1 - от мастера, 2 - от рабочих
    ...
    if (rank == 0) { //Мастер генерирует 
        ...
        //    B[i] = Rrand(-100, 100);
        //}
        B = FindY(A, X2, N);//Нахождение свободный весов для X2
    }
    ...
    MPI_Scatter(A, blockSize * N, MPI_DOUBLE, subA, blockSize * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Scatter(B, blockSize, MPI_DOUBLE, subB, blockSize, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    MPI_Gather(subA, blockSize * N, MPI_DOUBLE, A, blockSize * N, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    MPI_Gather(subB, blockSize, MPI_DOUBLE, B, blockSize, MPI_DOUBLE, 0, MPI_COMM_WORLD);
